            ...
            string host = "localhost", int port = 8021, string password = "ClueCon", TimeSpan? timeout = null)
        {
            try
            {
                ...
                await socket.Messages.Where(
                          ...
                          .Do(_ => Console.WriteLine("Received Auth Request"), ex => Console.WriteLine("Error waiting for AuthRequest." + ex))
                          .ToTask()
                          .ConfigureAwait(false);

                if (firstMessage.ContentType == ContentTypes.RudeRejection)
                {
                    Console.WriteLine("InboundSocket connection rejected ({0}).".Fmt(firstMessage.BodyText));
                ...
                if (!result.Success)
                {
                    Console.WriteLine("InboundSocket authentication failed ({0}).".Fmt(result.ErrorMessage));
                    throw new InboundSocketConnectionFailedException("Invalid password when trying to connect to {0}:{1}".Fmt(host, port));
                }

                Console.WriteLine("InboundSocket authentication succeeded.");
