...
namespace NEventSocket
{
    ...
    public class OutboundSocket : EventSocket
    {
        ...
        public async Task<ChannelEvent> Connect()
        {
            ...
            {
                Console.WriteLine("This socket is not using 'async' mode - certain dialplan applications may bock control flow");
            }

            if (controlMode != "full")
            {
                Console.WriteLine("This socket is not using 'full' control mode - FreeSwitch will not let you execute certain commands.");
            }

            Messages.FirstAsync(m => m.ContentType == ContentTypes.DisconnectNotice)
                            .Subscribe(dn => Console.WriteLine("Channel {0} Disconnect Notice {1} received.".Fmt(ChannelData.UUID, dn.BodyText)));
