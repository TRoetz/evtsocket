...
namespace NEventSocket.Sockets
{
    ...
    public abstract class EventSocket : ObservableSocket
    {
        ...
        protected EventSocket(TcpClient tcpClient, TimeSpan? responseTimeOut = null) : base(tcpClient)
        {
            ...
            messages =
                Receiver.SelectMany(x => Encoding.UTF8.GetString(x))
                    ...
                    .Do(
                        x => Console.WriteLine("Messages Received [{0}].".Fmt(x.ContentType)),
                        ex => { },
                        () =>
                        {
                            Console.WriteLine("Messages Observable completed.");
                            Dispose();
                        })
            ...
            Console.WriteLine("EventSocket initialized");
        }
        ...
        public async Task<ApiResponse> SendApi(string command)
        {
            ...
            {
                Console.WriteLine("Sending [api {0}]".Fmt(command));
                            ...
                            .Do(
                                m =>
                                {
                                    if (m.Success && command.StartsWith("uuid_dump"))
                                    {
                                        ...
                                        Console.WriteLine("ApiResponse received CHANNEL_DATA for [{0}]".Fmt(command));
                                    }
                                    else
                                    {
                                        Console.WriteLine("ApiResponse received [{0}] for [{1}]".Fmt(m.BodyText.Replace("\n", string.Empty), command));
                                    }
                                },
                                ex => Console.WriteLine("Error waiting for Api Response to [{0}].".Fmt(command), ex))
        ...
        public async Task<CommandReply> SendCommand(string command)
        {
            using (var asyncLock = await gate.LockAsync().ConfigureAwait(false))
            {
                Console.WriteLine("Sending [{0}]".Fmt(command));
                ...
                subscriptions.Add(
                    Messages.Where(x => x.ContentType == ContentTypes.CommandReply)
                            ...
                            .Do(
                                ...
                                ex => Console.WriteLine("Error waiting for Command Reply to [{0}].".Fmt(command), ex))
                            .Subscribe(x => tcs.TrySetResult(x), ex => tcs.TrySetException(ex), subscriptions.Dispose));
        ...
        public Task<BackgroundJobResult> BackgroundJob(string command, string arg = null, Guid? jobUUID = null)
        {
            ...
            subscriptions.Add(
                Events.Where(x => x.EventName == EventName.BackgroundJob && x.Headers[HeaderNames.JobUUID] == jobUUID.ToString())
                        ...
                        .Do(result => Console.WriteLine("BackgroundJobResult received [{0}] for [{1}]".Fmt(result.BodyText.Replace("\n", string.Empty), command)),
                            ex => Console.WriteLine("Error waiting for BackgroundJobResult Reply to [{0}].".Fmt(command), ex))
                        .Subscribe(x => tcs.TrySetResult(x), ex => tcs.TrySetException(ex), subscriptions.Dispose));
            ...
            lock (gate)
            {
                ...
                subscriptions.Add(
                    Messages.Where(x => x.ContentType == ContentTypes.CommandReply)
                            ...
                            .Subscribe(
                                result =>
                                Console.WriteLine("CommandReply received [{0}] for [{1}]".Fmt(result.ReplyText.Replace("\n", string.Empty), command)),
                                ex =>
                                    {
                                        Console.WriteLine("Error waiting for Command Reply to [{0}].".Fmt(command), ex);
                ...
                subscriptions.Add(
                    Messages.Where(x => x.ContentType == ContentTypes.DisconnectNotice)
                            ...
                            .Subscribe(
                                x =>
                                    {
                                        Console.WriteLine("Disconnect Notice received [{0}]".Fmt(x.BodyText));
                                        tcs.TrySetResult(x);
                                    },
                                ex =>
                                    {
                                        Console.WriteLine("Error waiting for Disconnect Notice" + ex);
