...
namespace NEventSocket.Sockets
{
    ...
    public abstract class ObservableListener<T> : IDisposable where T : ObservableSocket
    {
        ...
        public void Start()
        {
            ...
            Console.WriteLine("Listener Started on Port {0}".Fmt(Port));

            subscription = Observable.FromAsync(tcpListener.AcceptTcpClientAsync)
                .Repeat()
                .TakeUntil(listenerTermination)
                .Do(connection => Console.WriteLine("New Connection from {0}".Fmt(connection.Client.RemoteEndPoint)))
                .Select(
                    tcpClient =>
                    {
                        try
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Console.WriteLine("Unable to create observableSocket", ex);
                            return null;
                        }
                    })
                .Where(x => x != null)
                .Subscribe(
                    ...
                    ex =>
                    {
                        ...
                        {
                            Console.WriteLine("Error handling inbound connection", ex);
                        }
                    },
                    () => isStarted.Set(false));
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                Console.WriteLine("Disposing (disposing:{0})".Fmt(disposing));
