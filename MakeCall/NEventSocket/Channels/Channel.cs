...
namespace NEventSocket.Channels
{
    ...
    public class Channel : BasicChannel
    {
        ...
        public async Task BridgeTo(string destination, BridgeOptions options, Action<EventMessage> onProgress = null)
        {
            ...
            Console.WriteLine("Channel {0} is attempting a bridge to {1}".Fmt(UUID, destination));
            ...
            Console.WriteLine("Channel {0} bridge complete {1} {2}".Fmt(UUID, result.Success, result.ResponseText));
        ...
        private void InitializeSubscriptions()
        {
            ...
            {
                Console.WriteLine("Channel already initialized");
                return;
            }
            ...
            Disposables.Add(
                    ...
                    .Subscribe(
                        async x =>
                        {
                            Console.WriteLine("Channel [{0}] Bridged to [{1}] CHANNEL_BRIDGE".Fmt(UUID, x.GetHeader(HeaderNames.OtherLegUniqueId)));
                            ...
                            {
                                Console.WriteLine("Unable to get CHANNEL_DATA info from 'api uuid_dump {0}' - received '{1}'.".Fmt(x.OtherLegUUID, apiResponse.BodyText));
                            }
                        }));
                ...
                eventSocket.ChannelEvents.Where(x => 
                           ...
                           .Subscribe(
                               x =>
                               {
                                   ...
                                   Console.WriteLine("Channel [{0}] Unbridged from [{1}] {2}".Fmt(
                                               UUID,
            ...
            Disposables.Add(BridgedChannels.Subscribe(
                async b =>
                {
                    ...
                    Console.WriteLine("Channel [{0}] setting OtherLeg to [{1}]".Fmt(UUID, b.UUID));
                }));
                ...
                eventSocket.ChannelEvents.Where(
                    ...
                    .Subscribe(
                        x =>
                        {
                            ...
                            Console.WriteLine("Channel [{0}] bridged to [{1}]] on CHANNEL_BRIDGE received on other channel".Fmt(UUID, x.UUID));
                               ...
                               .Subscribe(
                                   async e =>
                                   {
                                       if (ExitOnHangup)
                                       {
                                           ...
                                           if (LingerTime > 0)
                                           {
                                               Console.WriteLine("Channel[{0}] will exit in {1} seconds...".Fmt(UUID, LingerTime));
                                               await Task.Delay(LingerTime * 1000);
                                           }

                                           if (eventSocket != null)
                                           {
                                               Console.WriteLine("Channel [{0}] exiting".Fmt(UUID));
            ...
            Console.WriteLine("Channel [{0}] subscriptions initialized".Fmt(UUID));
        }
    }
}