    ...
    public abstract class BasicChannel
    {
                               ...
                               e =>
                                   {
                                       ...
                                       {
                                           Console.WriteLine("Channel [{0}] Answered".Fmt(UUID));
                                       }

                                       if (e.EventName == EventName.ChannelHangupComplete)
                                       {
                                           Console.WriteLine("Channel [{0}] Hangup Detected [{1}]".Fmt(UUID, e.HangupCause));
                                           ...
                                           {
                                               Console.WriteLine("Channel [{0}] error calling hangup callback".Fmt(UUID), ex);
                                           }
        ...
        public IObservable<string> FeatureCodes(string prefix = "#")
        {
            return eventSocket
                       ...
                       .Do(x => Console.WriteLine("Channel {0} detected Feature Code {1}".Fmt(UUID, x)));
        }
        ...
        public async Task<IDisposable> PlayUntilCancelled(string file)
        {
            if (!CanPlayBackAudio)
            {
                Console.WriteLine("Channel [{0}] attempted to play hold music when not answered".Fmt(UUID));
            ...
            var cancellation = new DisposableAction(
                async () =>
                {
                    ...
                    {
                        Console.WriteLine("Error calling 'api uuid_break {0}'".Fmt(UUID), ex);
                    }
                });
            ...
            try
            {
                ...
                subscriptions.Add(channelExecuteComplete.Where(x => x.GetVariable("originate_disposition") != "SUCCESS")
                    .Subscribe(
                        x =>
                        {
                            Console.WriteLine("Att Xfer Not Answered");
                ...
                subscriptions.Add(Observable.When(cAnsweredThenHungUp.Then((answer, hangup, execComplete) => new { answer, hangup, execComplete }))
                                            .Subscribe(
                                                x =>
                                                {
                                                    Console.WriteLine("Att Xfer Rejected after C Hungup");
                ...
                subscriptions.Add(channelExecuteComplete.Where(x => !string.IsNullOrEmpty(x.GetVariable("xfer_uuids")))
                                            .Subscribe(x => {
                                                    Console.WriteLine("Att Xfer Success (threeway)");
                ...
                subscriptions.Add(Observable.When(cAnsweredThenBPressedStarOrHungUp.Then((answer, hangup, bridge) => new { answer, hangup, bridge }))
                                            .Subscribe(
                                                x =>
                                                {
                                                    Console.WriteLine("Att Xfer Succeeded after B pressed *");
                ...
                subscriptions.Add(Observable.When(bLegHangup.And(cLegAnswer).And(aLegBridge.Where(x => x.OtherLegUUID != bLegUUID)).Then((hangup, answer, bridge) => new { answer, hangup, bridge }))
                                            .Subscribe(
                                                x =>
                                                {
                                                    Console.WriteLine("Att Xfer Succeeded after B hung up and C answered");
                                                    tcs.TrySetResult(AttendedTransferResult.Success());
                                                }));
                ...
                subscriptions.Add(aLegHangup.Subscribe(
                    x =>
                    {
                        Console.WriteLine("Att Xfer Failed after A-Leg Hung Up");
        ...
        public Task SetChannelVariable(string name, string value)
        {
            return RunIfAnswered(
                () =>
                {
                    Console.WriteLine("Channel {0} setting variable '{1}' to '{2}'".Fmt(UUID, name, value));
        ...
        public Task StartRecording(string file, int? maxSeconds = null)
        {
            return RunIfAnswered(
                async () =>
                {
                    ...
                    {
                        Console.WriteLine("Channel {0} received a request to record to file {1} while currently recording to file {2}. Channel will stop recording and start recording to the new file."
                    ...
                    Console.WriteLine("Channel {0} is recording to {1}".Fmt(UUID, recordingPath));
            ...
            return RunIfAnswered(
                async () =>
                {
                    ...
                    else
                    {
                        ...
                        Console.WriteLine("Channel {0} has unmasked recording to {1}".Fmt(UUID, recordingPath));
                        recordingStatus = RecordingStatus.Recording;
                    }
                });
        }
            ...
            return RunIfAnswered(
                async () =>
                {
                    ...
                    else
                    {
                        ...
                        recordingPath = null;
                        Console.WriteLine("Channel {0} has stopped recording to {1}".Fmt(UUID, recordingPath));
